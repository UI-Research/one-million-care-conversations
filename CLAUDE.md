...
- **Branches are `iss<n>`** for issue n; PRs close the issue. The rendered site (`docs/`, from `quarto render` at the root) and `_freeze/` are committed; `quarto publish gh-pages --no-render` pushes `docs/` to the `gh-pages` branch that GitHub Pages serves (publishing is a deliberate, outward-facing step — confirm with Manu first). The coding dashboard is a site resource built into `coding-dashboard/` by `scripts/nlp/coding_dashboard.R`.
- **Files are classified from the DATA LOG, not from file names.** `00_ingest-raw.R` parses the log's Details sheet into `data/raw/deliveries.csv` (tool, form, closed/open, complete/partial, test, collection window, ID range); the file name is a second opinion that only fills gaps and raises a warning on disagreement. The read chunk in 01 takes survey / canvassing / revised-canvassing files from that table, requires every mirrored file to be either cleaned or set aside with a reason, and `check_against_log()` warns when a file's dates or IDs fall outside what the log says. Nothing in `data/raw/` is renamed by hand. The log's tool-type text is hand-typed — extend the parser's rules in the ingest script when a new value appears (it stops on unknown ones).
