...
Every file in the mirror gets exactly one fate: claimed by a cleaning section
here, recognized as a known set-aside, or a loud error. That way a renamed or
brand-new delivery type can never be skipped silently — the render stops and
the fix is a deliberate pattern or set-aside update.
...
# selected (and drift-guarded) alongside the survey files in the read chunk
