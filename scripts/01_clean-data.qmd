...
decides which questions they see next. There are five pathways:

- `current`: giving or receiving care now
- `past`: cared for someone in the past
- `future`: expects to give or need care in the next five years
- `observer`: knows someone who receives or needs care
- `none`: none of these

According to the questionnaire, a person who ticks answers from more than
one group should be routed to the first that applies, in that order. The
data mostly bear that out (a person eligible for both `past` and `future` was
shown only the `past` questions), with one exception: paid care providers who
also cared for someone in the past were shown both the `current` and the
`past` questions. We have not yet confirmed with Caring Across Generations
whether that is intended. The cleaning assigns those respondents to
`current`.
...
outside their pathway count against the check. The one known exception: paid
care providers who also cared for someone in the past were shown two sets of
questions, so they appear as off-pathway answers on the past questions.
