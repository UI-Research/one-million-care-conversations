...
bar_rows <- vapply(seq_len(nrow(counts)), function(i) {
  ...
  sprintf('<div class="brow" title="%s">
   ...
   </div>', esc(tip), esc(cd), 100 * n / max_n, domain_colors[[counts$domain[i]]], n, sum(pf > 0))
}, character(1))
...
overview <- sprintf('<section class="tx-section panel" id="sec-overview">
  ...
  <div class="meta">%d coded segments across %d transcripts &middot; %d distinct codes</div>
...
page <- sprintf('<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
...
</script></body></html>', tabs, legend, overview, codebook_tab, sections)
